// added nice command to change nice value
// modified to say that no runnable processes are available if all processes are sleeping 
...
enum cmds        { FORK=0, SETPID,   SHOWPID,    WAIT,   EXIT,   SLEEP,   WAKEUP,   PS,   SCHEDULE,   TIMER,   HELP,  NICE,   QUIT };
char *cmdstr[] = {"fork", "Setpid", "currpid",  "wait", "exit", "sleep", "wakeup", "ps", "schedule", "timer", "help", "nice", "quit"};
...
int executeCmd(char** params, int nparams)
{
    ...
    case NICE:
        if (nparams < 3)
        {
            printf("Usage: nice [pid] [nice value]\n");
        }
        else 
        {
            if (Nice(atoi(params[1]), atoi(params[2])) == 0)
            {
                printf("Nice value set.\n");
            }
            else
            {
    ...
    case HELP:
        printf("Commands: fork, wait, exit, ps, Setpid, currpid, sleep, wakeup, timer, nice, schedule, help\n");
        break;
