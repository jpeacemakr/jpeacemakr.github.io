...
int get_opts(int count, char *args[]) {
    ...
    while (good && (opt = getopt(count, args, "s:l:e:")) != -1) {
        ...
        switch (opt) {
            ...
            //this is the option i am adding. option for end block
            case 'e':
                len = strlen(optarg);
                for (i=0;i<len; i++)
                    if (!isdigit(optarg[i])) {
                        fprintf(stderr, "-e value must be a number.\n");
                        good = 0;
                        break;
                    }
                if (good)
                {
                    ...
                    if (blocks < 0) {
                        fprintf(stderr, "-s must be less than -e.\n");
                        good = 0;
                        break;
                    }
                }
...
//reads a block of data and puts it in buffer
...
//opens a file and assigns file descriptor to fs
