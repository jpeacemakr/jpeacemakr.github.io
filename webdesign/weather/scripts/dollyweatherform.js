...
function badLocation() {
 ...
 // changes the walk parameters based on what is in the form
 function changeWalkParameters(){
  event.preventDefault();

  walkStartTime = document.getElementById("earlywalktime").value;
  walkEndTime = document.getElementById("latewalktime").value;
  walkPrecipitation = document.getElementById("precipchance").value;
 ...
 // fetch the hourly weather forecast
 ...
 // prints the hourly weather forecast
 function printWeatherHourly() {
  ...
  hourlyForecastText.innerHTML = `
   ...
   <form action="" onsubmit="changeWalkParameters()">
      
      <div class="row">
        <div class="form-group col-md-4">
          <label for="earlywalktime">Earliest walk time:</label>
          <select id="earlywalktime" name="earlywalktime" class="form-control">
        ...
        <div class="form-group col-md-4">
          <label for="latewalktime">Latest walk time:</label>
          <select id="latewalktime" name="latewalktime" class="form-control">
            <option value="0">Midnight</option>
            <option value="1">1 a.m.</option>
            <option value="2">2 a.m.</option>
            <option value="3">3 a.m.</option>
        ...
        <div class="form-group col-md-4">
          <label for="precipchance">Precipitation:</label>
          <select id="precipchance" name="precipchance" class="form-control">
            <option value="0">I don't want to get wet.</option>
            <option value="1">A slight chance is OK.</option>
            <option value="2">I will roll the dice.</option>
            <option value="3">I don't mind getting wet.</option>
            <option value="4">Angry sky fluffs don't scare me!</option>
          </select>
        </div>
  ...
  // form
  // set walk time
  document.getElementById("earlywalktime").value = walkStartTime;
  document.getElementById("latewalktime").value = walkEndTime;
  
  if ( parseInt(walkEndTime) < parseInt(walkStartTime) ) {
    alert("Grrrrr. Walk start time can't be later than then the end time.");
  } 

  // set temp range
  // low temp
  var lowSlider = document.getElementById("lowtempslider");
  lowSlider.value = walkTempMin;
  var lowOutput = document.getElementById("lowtempoutput");
  lowOutput.innerHTML = walkTempMin;

  lowSlider.oninput = function() {
    lowOutput.innerHTML = this.value;
    walkTempMin = this.value;
  }

  // high temp
  var highSlider = document.getElementById("hightempslider");
  highSlider.value = walkTempMax;
  var highOutput = document.getElementById("hightempoutput");
  highOutput.innerHTML = walkTempMax;

  highSlider.oninput = function() {
    highOutput.innerHTML = this.value;
    walkTempMax = this.value;
  }

  if ( parseInt(walkTempMax) < parseInt(walkTempMin) ) {
    alert("Grrrrr. The minimum temperature can't be lower than the maximum.");
  } 

  //set acceptable chance of precipitation
  document.getElementById("precipchance").value = walkPrecipitation;
  ...
  for (i=0; i <weatherResultHourly.properties.periods.length; i++) {
    ...
    // get weather severety number
    // if slight 1, chance 2, showers 3, storms 4, else 0
    let precipitationNumber = 0;
    if ( weatherResultHourly.properties.periods[i].shortForecast.includes("Slight") ) {
      precipitationNumber = 1;
    } else if ( weatherResultHourly.properties.periods[i].shortForecast.includes("Chance") ) {
      precipitationNumber = 2;
    }  else if ( weatherResultHourly.properties.periods[i].shortForecast.includes("Shower") ) {
      precipitationNumber = 3;
    }  else if ( weatherResultHourly.properties.periods[i].shortForecast.includes("Thunderstorm") ) {
      precipitationNumber = 4;
    }
